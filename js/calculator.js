...
class Calculator {
  ...
  calcTripPnL(trip, settings) {
    ...
    // Wear
    const wearCost = (settings.mode === 'ownerOp')
      ? this.calcWearCost({
    ...
    // Depreciation (owner-op only)
    const depreciation = (settings.mode === 'ownerOp' && settings.truckPrice && settings.truckLifeKm)
    ...
    const totalExpenses = fuelCost + wearCost + depreciation + tolls + fines + ferry + driverWage;
    const profit        = Math.round((income - totalExpenses) * 100) / 100;

    return { income, fuelCost, wearCost, depreciation, tolls, fines, ferry, driverWage, totalExpenses, profit };
  }
